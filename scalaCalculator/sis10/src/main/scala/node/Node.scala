...
import service.{FibonacciService, WordCountService}
      ...
      Behaviors.receiveMessage { message => {
        message match {
          case FindNumberPing(n, replyTo) =>
            ps ! FibonacciService.Ping(n, replyTo)
          ...
          case CountWordFrom(text, replyTo) =>
            ...
            //              cm ! ClusterManager.GetMessage(text, replyTo)
            countService ! WordCountService.Ping(text, replyTo)
          //              context.ask[Receptionist.Command,Receptionist.Listing](context.system.receptionist, Receptionist.Find(NodeServiceKey, _)){
